...
const main = async () => {
    ...
    const amountOut = ethers.parseUnits("2000000000000000",6);
    ...
    const WthBalAfterSwap = await WETH.balanceOf(impersonatedSigner.address);
    const ethBalAfterSwapBal = await impersonatedSigner.provider.getBalance(USDCHolder);
    ...
    console.log("wth balance after swap", ethers.formatUnits(WthBalAfterSwap, 18) );
    console.log("ETH Balance:", ethers.formatUnits(ethBalAfterSwapBal,18));
    // console.log("ETH Balance:", ethers.formatUnits(ethBalAfterSwap,18));
